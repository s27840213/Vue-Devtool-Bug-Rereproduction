  ...
  div(class="panel-font-format")
    div(class="panel-font-format__bar mb-15")
      svg-icon(v-for="(icon,index) in mappingIcons('font')"
        ...
        :id="`icon-${icon}`"
        :iconName="icon" :iconWidth="'20px'" :iconColor="iconIsActived(icon)? 'blue-1' :iconClickable(icon) ? 'gray-2' : 'gray-4'" @mousedown.native="onPropertyClick(icon)")
    div(class="panel-font-format__bar")
      svg-icon(v-for="(icon,index) in mappingIcons('font-align')"
        class="pointer feature-button p-5"
        :key="`gp-action-icon-${index}`"
        :iconName="icon" :iconWidth="'20px'" :iconColor="iconIsActived(icon)? 'blue-1'  :'gray-2'" @mousedown.native="onParaPropsClick(icon)")
</template>
