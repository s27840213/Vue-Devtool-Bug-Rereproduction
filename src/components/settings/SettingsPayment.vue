<template lang="pug">
  div(class="sp")
    ...
    div(class="sp-plan")
      ...
      span(v-if="showDueDay")           {{$t('TMP0090', {date: myPaidDate})}}
      ...
      span(v-if="canSwitch" class="text-blue-1 pointer"
          @click="switchPeriod()")      {{$t('TMP0088', { period: isBundle ? $t('TMP0010') : $t('TMP0011')})}}
      span(v-if="canCancel" class="text-gray-3 pointer"
          @click="cancelSub()")         {{$t('TMP0089')}}
    div(v-if="showUsage" class="sp-usage")
      span(class="text-blue-1 mt-30")   {{$t('TMP0091')}}
      span(                       v-html="$t('TMP0092', { amount: usage.bgrmRemain, date: myPaidDate })")
    ...
    div(v-if="showBillingInfo" class="sp-info")
      span(class="text-blue-1 body-MD") {{$t('TMP0099')}}
      //- switch(input.label)
      template(v-for="input in billingInfoInput")
        ...
        div(v-else-if="input.label === 'state & zip'" class="sp-info__half")
          span
          label(for="zip") {{$t('TMP0111')}}
          options(:options="stateData"
                  v-model="bi.state" :ph="$t('TMP0110')")
          input(id="zip" v-model="bi.zip" :invalid="biv.zip"
                :placeholder="$t('TMP0111')")
          span
