  ...
  div(class="sp")
    ...
    div(v-if="isPro" class="sp-pro")
      ...
      template(v-if="isCancelingPro")
        span(class="body-SM")           {{$t('TMP0081', {date: nextPaidDate})}}
        btn(class="rounded" type="primary-mid" @click.native="resume()")
          span                          {{$t('TMP0076')}}
      template(v-else)
        span(class="body-SM")           {{$t('TMP0077', { period: isBundle ? $t('TMP0011') : $t('TMP0010') })}}
        span(class="body-SM"      v-html="$t('TMP0078', { price: nextPrice, date: nextPaidDate  })")
        span(class="text-blue-1 body-SM pointer"
            @click="switchPeriod()")    {{$t('TMP0079', { period: isBundle ? $t('TMP0010') : $t('TMP0011')})}}
        span(class="text-gray-3 body-SM pointer"
            @click="cancelSub()")       {{$t('TMP0080')}}
      span(class="text-blue-1 mt-30")   {{$t('TMP0082')}}
      span(                       v-html="$t('TMP0083', { amount: usage.bgrmRemain, date: nextPaidDate })")
      span(class="body-XS")             {{$t('TMP0084')}}
      span(class="text-blue-1")         {{$t('TMP0085')}}
    ...
    div(v-if="isPro" class="sp-detail")
      ...
      span(class="body-SM")             {{`···· ···· ···· ${card.last4}  ${$t('TMP0088')} ${card.date}`}}
      p(class="text-blue-1 body-SM"
        @click="openCardPopup()")       {{$t('TMP0089')}}
