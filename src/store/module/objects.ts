...
const actions: ActionTree<IListModuleState, unknown> = {
  async getContent ({ commit, state }, params = {}) {
    ...
    try {
      const { data } = await list.getSvg({ locale, category })
  ...
  async getMoreContent ({ commit, getters }) {
    ...
    try {
      const { data } = await list.getSvg(nextParams)
...
const getters: GetterTree<IListModuleState, any> = {
  ...
  emptyResultMessage (state) {
    ...
    return query
      ? `Sorry, we couldn't find any object for "${query}". Try searching something related.`
      : 'Sorry, we couldn\'t find any object.'
  }
}
