  ...
  select(layerIndexs: Array<number>) {
    ...
    if (store.getters.getCurrSelectedIndex < 0) {
      if (layerIndexs.length === 1) {
        ...
        const tmpIndex = topIndex - newLayersNum + 1
        store.commit('SET_currSelectedInfo', {
          index: tmpIndex,
          layers: tmpLayers,
          types: calcType(tmpLayers)
      ...
      if (store.getters.getCurrSelectedLayers.length === 1) {
        ...
        const layers = MappingUtils.mappingLayers(layerIndexs)
        const prevTmpStyles = getTmpStyles(store.getters.getCurrSelectedIndex)
        const tmpStyles = calcTmpProps([...this.mapLayersToPage(store.getters.getCurrSelectedLayers, getTmpStyles(store.getters.getCurrSelectedIndex)), ...layers])
  ...
  deselect() {
    ...
    if (store.getters.getCurrSelectedIndex !== -1) {
      if (store.getters.getCurrSelectedLayers.length === 1) {
        ...
        const tmpStyles = getTmpStyles(store.getters.getCurrSelectedIndex)
