    ...
    ps.forEach((p, pIndex) => {
      ...
      for (const [sIndex, el] of p.childNodes.entries()) {
        ...
        spanStyle = {
          ...
          size: spanEl.style.fontSize ? Math.round(parseFloat(spanEl.style.fontSize.split('px')[0]) / (4 / 3) * 100) / 100
      ...
      if (spans.length) {
        ...
        const fontSize = Math.round(parseFloat(pEl.style.fontSize.split('px')[0]) / (4 / 3) * 100) / 100
