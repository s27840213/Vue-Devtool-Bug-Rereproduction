...
const getDefaultState = (): IEditorState => ({
  ...
  isTablet: window.matchMedia(
    'screen and (min-width: 768px) and (orientation: portrait), screen and (min-height: 768px) and (orientation: landscape)',
  ).matches,
...
const getters: GetterTree<IEditorState, unknown> = {
  ...
  getLayer(state: IEditorState) {
    return (
      pageIndex: number,
      layerIndex: number,
    ): IShape | IText | IImage | IGroup | IFrame | ITmp => {
...
const mutations: MutationTree<IEditorState> = {
  ...
  SET_pages(
    ...
    if (Array.isArray(newPageConfigs)) {
      ...
      state.pages = (
        newPageConfigs.loadDesign ? pageUtils.newPages(newPageConfigs.pages) : newPageConfigs.pages
  ...
  SET_folderInfo(
    state: IEditorState,
    folderInfo: { isRoot: boolean; parentFolder: string; path: string },
  ) {
  ...
  UPDATE_pageProps(
    state: IEditorState,
    updateInfo: { pageIndex: number; props: { [key: string]: string | number } },
  ) {
    /**
  ...
  SET_layers(
    state: IEditorState,
    updateInfo: { pageIndex: number; newLayers: Array<IShape | IText | IImage | IGroup> },
  ) {
  ...
  SET_pageScaleRatio(state: IEditorState, ratio: number) {
    state.pageScaleRatio = generalUtils.isStk ? 100 : ratio
  },
  ...
  SET_backgroundColor(state: IEditorState, updateInfo: { pageIndex: number; color: string }) {
    ...
    state.pages[updateInfo.pageIndex].config.backgroundImage.config.srcObj = {
      type: '',
      userId: '',
      assetId: '',
    }
  ...
  SET_backgroundImage(
    state: IEditorState,
    updateInfo: { pageIndex: number; config: IImage; bgColorReset?: boolean },
  ) {
  ...
  SET_backgroundImageSrc(
    state: IEditorState,
    updateInfo: { pageIndex: number; srcObj: any; previewSrc?: string; panelPreviewSrc: '' },
  ) {
    ...
    panelPreviewSrc &&
      (state.pages[pageIndex].config.backgroundImage.config.panelPreviewSrc = panelPreviewSrc)
    state.pages[pageIndex].config.backgroundColor = '#ffffff'
    // state.pages[updateInfo.pageIndex].backgroundColor = '#ffffff'
  },
  SET_backgroundImagePos(
    state: IEditorState,
    updateInfo: { pageIndex: number; imagePos: { x: number; y: number } },
  ) {
  ...
  SET_backgroundImageStyles(
    state: IEditorState,
    updateInfo: { pageIndex: number; styles: Partial<IImageStyle> },
  ) {
    const { pageIndex, styles } = updateInfo
    Object.assign(state.pages[pageIndex].config.backgroundImage.config.styles, styles)
  },
  SET_backgroundImageMode(
    state: IEditorState,
    updateInfo: { pageIndex: number; newDisplayMode: boolean },
  ) {
    state.pages[updateInfo.pageIndex].config.backgroundImage.newDisplayMode =
      updateInfo.newDisplayMode
  },
  SET_backgroundImageControl(
    state: IEditorState,
    updateInfo: { pageIndex: number; imgControl: boolean },
  ) {
    state.pages[updateInfo.pageIndex].config.backgroundImage.config.imgControl =
      updateInfo.imgControl
  },
  ...
  SET_backgroundOpacity(state: IEditorState, updateInfo: { pageIndex: number; opacity: number }) {
    state.pages[updateInfo.pageIndex].config.backgroundImage.config.styles.opacity =
      updateInfo.opacity
  },
  ...
  ADD_newLayers(
    state: IEditorState,
    updateInfo: { pageIndex: number; layers: Array<IShape | IText | IImage | IGroup> },
  ) {
  ...
  ADD_layersToPos(
    state: IEditorState,
    updateInfo: { pageIndex: number; layers: Array<IShape | IText | IImage | IGroup>; pos: number },
  ) {
  ...
  DELETE_subLayer(
    state: IEditorState,
    updateInfo: { pageIndex: number; primaryIndex: number; subIndex: number },
  ) {
  ...
  REPLACE_layer(
    ...
    updateInfo: {
      pageIndex: number
      layerIndex: number
      layer: IShape | IGroup | IImage | IText | ITmp
    },
  ) {
    ...
    updateInfo: {
      ...
      props: {
        [key: string]:
          ...
          | IParagraph
          | Array<string>
          | Array<IShape | IText | IImage | IGroup>
          | number[]
          | SrcObj
      }
    },
  ...
  UPDATE_subLayerProps(
    state: IEditorState,
    updateInfo: {
      pageIndex: number
      layerIndex: number
      targetIndex: number
      props: { [key: string]: string | number | boolean | IParagraph | SrcObj }
    },
  ) {
    const groupLayer = state.pages[updateInfo.pageIndex].config.layers[
      updateInfo.layerIndex
    ] as IGroup
  ...
  DELETE_subLayerProps(
    state: IEditorState,
    updateInfo: { pageIndex: number; layerIndex: number; targetIndex: number; propKeys: string[] },
  ) {
    const groupLayer = state.pages[updateInfo.pageIndex].config.layers[
      updateInfo.layerIndex
    ] as IGroup
  ...
  UPDATE_frameLayerProps(
    state: IEditorState,
    updateInfo: {
      pageIndex: number
      layerIndex: number
      targetIndex: number
      props: { [key: string]: string | number | boolean | SrcObj }
    ...
    if (preprimaryLayerIndex !== -1) {
      if (state.pages[pageIndex].config.layers[preprimaryLayerIndex].type === LayerType.group) {
        frame = (state.pages[pageIndex].config.layers[preprimaryLayerIndex] as IGroup).layers[
          layerIndex
        ]
      }
    } else {
  ...
  UPDATE_groupLayerProps(
    state: IEditorState,
    updateInfo: { props: { [key: string]: string | number | boolean | number[] } },
  ) {
    Object.entries(updateInfo.props).forEach(([k, v]) => {
      ;(
        state.pages[state.middlemostPageIndex].config.layers[state.currSelectedInfo.index] as IGroup
      ).layers.forEach((layer) => {
        layer[k] = v
      })
    })
  },
  UPDATE_selectedLayerProps(
    state: IEditorState,
    updateInfo: {
      pageIndex: number
      layerIndex: number
      props: { [key: string]: string | number | boolean | Array<string> }
    },
  ) {
    Object.entries(updateInfo.props).forEach(([k, v]) => {
      ;(
        state.pages[state.currSelectedInfo.pageIndex].config.layers[
          state.currSelectedInfo.index
        ] as ITmp
      ).layers[updateInfo.layerIndex][k] = v
    })
  },
  UPDATE_textProps(
    state: IEditorState,
    ...
    ;(state.pages[updateInfo.pageIndex].config.layers[updateInfo.layerIndex] as IText).paragraphs =
      updateInfo.paragraphs
  },
  UPDATE_paragraphStyles(
    state: IEditorState,
    updateInfo: {
      pageIndex: number
      layerIndex: number
  ...
  UPDATE_layerOrders(state: IEditorState, updateInfo: { pageIndex: number }) {
    state.pages[updateInfo.pageIndex].config.layers.sort(
  ...
  UPDATE_layerOrder(state: IEditorState, updateInfo: { type: string }): void {
    ...
    switch (updateInfo.type) {
      case 'front': {
        const layer = state.pages[state.currSelectedInfo.pageIndex].config.layers.splice(
          layerIndex,
          1,
        )
      ...
      case 'forward': {
        ...
        state.pages[state.currSelectedInfo.pageIndex].config.layers.splice(
          layerIndex + 1,
          0,
          ...layer,
        )
  ...
  UPDATE_tmpLayerStyles(
    state: IEditorState,
    updateInfo: { pageIndex: number; styles: { [key: string]: string | number } },
  ) {
  ...
  UPDATE_groupLayerStyles(
    state: IEditorState,
    updateInfo: { styles: { [key: string]: string | number } },
  ) {
    Object.entries(updateInfo.styles).forEach(([k, v]) => {
      ;(
        state.pages[state.middlemostPageIndex].config.layers[state.currSelectedInfo.index] as IGroup
  ...
  UPDATE_selectedLayersStyles(
    state: IEditorState,
    updateInfo: { styles: { [key: string]: string | number }; layerIndex?: number },
  ) {
    Object.entries(updateInfo.styles).forEach(([k, v]) => {
      if (typeof updateInfo.layerIndex !== 'undefined') {
        ...
        ).layers[updateInfo.layerIndex].styles[k] = v
      } else {
        ;(
          state.pages[state.currSelectedInfo.pageIndex].config.layers[
            state.currSelectedInfo.index
    ...
    if (
      state.pages[state.currSelectedInfo.pageIndex].config.layers[state.currSelectedInfo.index]
        .type === 'group'
    ) {
      state.currSelectedInfo.layers = [
        state.pages[state.currSelectedInfo.pageIndex].config.layers[
          state.currSelectedInfo.index
        ] as IShape | IText | IImage | IGroup | IFrame,
      ]
    } else {
      state.currSelectedInfo.layers = (
        state.pages[state.currSelectedInfo.pageIndex].config.layers[
  ...
  UPDATE_selectedTextParagraphs(
    state: IEditorState,
    updateInfo: { tmpLayerIndex: number; paragraphs: [IParagraph] },
  ) {
    ;(
      (
        state.pages[state.currSelectedInfo.pageIndex].config.layers[
      ...
      ).layers[updateInfo.tmpLayerIndex] as IText
    ).paragraphs = updateInfo.paragraphs
    if (
      state.pages[state.currSelectedInfo.pageIndex].config.layers[state.currSelectedInfo.index]
        .type === 'group'
    ) {
  ...
  UPDATE_tmpLayersZindex(state: IEditorState) {
    const tmpLayer = state.pages[state.currSelectedInfo.pageIndex].config.layers[
      state.currSelectedInfo.index
    ] as ITmp
  ...
  SET_currSelectedInfo(state: IEditorState, data: ICurrSelectedInfo) {
    ...
    // }
    if (pageIndex === -1) {
      state.currFocusPageIndex =
        state.currActivePageIndex === -1 ? state.middlemostPageIndex : state.currActivePageIndex
    } else {
  ...
  SET_currSelectedResInfo(
    state: IEditorState,
    data: { userName: string; userLink: string; vendor: string; tags: string[] },
  ) {
    state.currSelectedResInfo = data
  },
  SET_subLayerStyles(
    state: IEditorState,
    data: { pageIndex: number; primaryLayerIndex: number; subLayerIndex: number; styles: any },
  ) {
    const { pageIndex, primaryLayerIndex, subLayerIndex, styles } = data
    const layers = state.pages[pageIndex].config.layers[primaryLayerIndex].layers as (
      | IShape
      | IText
      | IImage
    )[]
  ...
  SET_frameLayerAllClipsStyles(
    state: IEditorState,
    data: { pageIndex: number; primaryLayerIndex: number; styles: any },
  ) {
  ...
  SET_frameDecorColors(
    state: IEditorState,
    data: { pageIndex: number; layerIndex: number; subLayerIdx: number; payload: any },
  ) {
    const { pageIndex, layerIndex, subLayerIdx, payload } = data
    const { decorationColors, decorationTopColors } = payload
    const targetLayer =
      subLayerIdx === -1
        ? state.pages[pageIndex].config.layers[layerIndex]
        : (state.pages[pageIndex].config.layers[layerIndex] as IGroup).layers[subLayerIdx]
  ...
  SET_subFrameLayerAllClipsStyles(
    state: IEditorState,
    data: { pageIndex: number; primaryLayerIndex: number; subLayerIndex: number; styles: any },
  ) {
    const { pageIndex, primaryLayerIndex, subLayerIndex, styles } = data
  ...
  DELETE_previewSrc(
    state: IEditorState,
    { type, userId, assetId, assetIndex, forSticker = false },
  ) {
  ...
  ADD_guideline(
    state: IEditorState,
    updateInfo: { pos: number; type: string; pageIndex?: number },
  ) {
  ...
  DELETE_guideline(
    state: IEditorState,
    updateInfo: { pageIndex: number; index: number; type: string },
  ) {
  ...
  SET_isEnableBleed(state: IEditorState, payload: { value: boolean; pageIndex?: number }) {
    ...
    if (pageIndex) pages[pageIndex].config.isEnableBleed = value
      ...
      pages.forEach((page) => {
        page.config.isEnableBleed = value
      })
  },
  SET_bleeds(
    state: IEditorState,
    payload: { pageIndex: number; bleeds: IBleed; physicalBleeds: IBleed },
  ) {
    const { pages } = state
  ...
  UPDATE_documentColors(state: IEditorState, payload: { pageIndex: number; color: string }) {
    state.pages[payload.pageIndex].config.documentColors = getDocumentColor(
      payload.pageIndex,
      payload.color,
    )
  },
  ...
  UPDATE_pagePos(
    state: IEditorState,
    data: { pageIndex: number; styles: { [key: string]: number } },
  ) {
  ...
  UPDATE_frameInGroup(
    ...
    const frame = (state.pages[pageIndex].config.layers[primaryLayerIndex] as IGroup).layers[
      layerIndex
    ] as IFrame
    if (frame.type === LayerType.frame) {
      Object.entries(props).forEach(([k, v]) => {
...
window.addEventListener(
  'resize',
  throttle(() => store.commit('UPDATE_RWD'), 500),
)
