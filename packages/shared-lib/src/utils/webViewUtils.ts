...
export default abstract class WebViewUtils<T extends { [key: string]: any }> {
  ...
  async callIOSAsAPI(
    ...
    if (this.apiQueueMap[event] === undefined) {
      this.apiQueueMap[event] = []
    }
    if (cancelOnConfict) {
      if (this.callbackMap[event] !== undefined) {
  ...
  async callIOSAsAPICore(
    ...
    try {
      ...
      logUtils.setLog(
        `Error occurs in callIOSAsAPI with type: ${type}, message: ${message}, event: ${event}`
      )
      logUtils.setLogForError(error as Error)
      result = null
    }
