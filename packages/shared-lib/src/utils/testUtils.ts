...
class GeneralUtils {
  ...
  exact(conditions: Array<boolean>): boolean {
    return (
      conditions.filter((condition: boolean) => {
  ...
  scaleFromCenter(el: HTMLElement) {
    ...
    nextTick(() => {
      el.scrollLeft = Math.round(
        ((scrollCenterX * el.scrollWidth) / oldScrollWidth - el.clientWidth) / 2
      )
      el.scrollTop = Math.round(
        ((scrollCenterY * el.scrollHeight) / oldScrollHeight - el.clientHeight) / 2
      )
    })
  }
