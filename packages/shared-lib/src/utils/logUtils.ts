...
class LogUtils {
  ...
  setLog(logContent: string, trimLog = true) {
    ...
    try {
      if (this.isUploadingLog) {
        // when log is uploading, append to the log buffer
        this.logBuffer = `${this.logBuffer}\n${newContent}`
        return
      } else if (this.logBuffer) {
        // when log is not uploading, append the log buffer back to localStorage
        this.appendBuffer()
      }
      localStorage.setItem('log', `${this.getLog()}\n${newContent}`)
    } catch (error) {
  ...
  setLogAndConsoleLog(...logContent: any[]) {
    ...
    logContent = logContent
      .map((lc) => (typeof lc === 'string' ? lc : JSON.stringify(lc)))
      .map((lc) => lc.substring(0, 500))
