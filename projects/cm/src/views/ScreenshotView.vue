...
export default defineComponent({
  ...
  methods: {
        ...
        switch (type) {
          ...
          case 'json': {
            ...
            const renderPage = () => {
              ...
              // this.$store.commit('SET_contentScaleRatio4Page', { pageIndex: 0, contentScaleRatio: Math.min(window.outerWidth / newSize.width, window.outerHeight / newSize.height) })
    ...
    clearBuffers() {
      ...
      cmWVUtils.isAnyIOSImgOnError = false
    ...
    onload() {
      ...
      if (this.mode === ScreenShotMode.PAGE) {
        if (cmWVUtils.isAnyIOSImgOnError) {
          // TODO: implement cmWV missing img handler
          // // Inform UIWeb to handle missing img
          // cmWVUtils.sendToIOS('INFORM_WEB', {
          //   info: {
          //     event: 'missing-image',
          ...
          this.handleDoneLoading(this.JSONcontentSize.width * this.contentScaleRatio, this.JSONcontentSize.height * this.contentScaleRatio)
        }
        ...
        this.handleDoneLoading(width, height)
      } else {
        this.handleDoneLoading(window.outerWidth, window.outerHeight)
      }
    },
