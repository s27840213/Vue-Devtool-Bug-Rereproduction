...
export default defineConfig({
  plugins: [
    ...
    transformPlugin({
      ...
      callbackArray: [(s: string) => s.replace(/(?<==".+)( as [\w<>|', ]+)/g, '')],
    }),
    vue(),
    vuei18n({
      // TODO: Check if this plugin will decrease bundle size.
  ...
  define: {
    ...
    'process.env': loadEnv('production', process.cwd(), ['VUE_APP', 'BITBUCKET']),
