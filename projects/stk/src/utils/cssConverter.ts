...
const styleMap = Object.assign(
  ...
  ...fontProps.map(
    (
      prop, // Transfer camelCase to dash-case by default
...
class CssConveter {
  convertTransformStyle(
    ...
    return {
      transform: cancel3D
        ? `translate(${x * factor}px, ${y * factor}px) rotate(${rotate}deg)`
        : `translate3d(${x * factor}px, ${y * factor}px, ${zindex}px) rotate(${rotate}deg)`,
    }
  }
  ...
  convertFontStyle(
    ...
    fontProps.forEach((prop) => {
      ...
      if (prop === 'size') {
        ...
        result[styleMap[prop]] =
          sourceStyles[prop] === 'bold'
            ? `calc(var(--base-stroke) + ${(sourceStyles.size as number) / 32}px)`
            : 'calc(var(--base-stroke))'
      } else if (prop === 'fontSpacing') {
        result[styleMap[prop]] =
          typeof sourceStyles[prop] === 'number'
            ? `${sourceStyles[prop]}em`
            : `${sourceStyles[prop]}`
      } else if (prop === 'font') {
      ...
      } else if (['min-width', 'min-height'].includes(prop)) {
        // For fixedWidth LetterBg
        result[prop] = `${sourceStyles[prop]}`
        result.display = 'inline-block'
        result['letter-spacing'] = '0'
        result['text-align'] = 'center'
      } else if (['lineHeight', 'opacity'].includes(prop)) {
        // Use sorce style value directly
        result[styleMap[prop]] = `${sourceStyles[prop]}`
      } else {
        // Defulat: number add px unit, other treat as string
        result[styleMap[prop]] =
          typeof sourceStyles[prop] === 'number'
            ? `${sourceStyles[prop]}px`
            : `${sourceStyles[prop]}`
      }
    })
    return result
  ...
  convertDefaultStyle(
    sourceStyles: IStyle | ITextStyle,
    cancel3D = false,
    factor = 1,
  ): { [key: string]: string } {
    const result: { [key: string]: string } = {}
    Object.assign(result, {
      width:
        typeof sourceStyles.width === 'number' ? `${sourceStyles.width * factor}px` : 'initial',
      height:
        typeof sourceStyles.height === 'number' ? `${sourceStyles.height * factor}px` : 'initial',
      ...(sourceStyles.opacity !== 100 && { opacity: `${sourceStyles.opacity / 100}` }),
      ...this.convertTransformStyle(
        sourceStyles.x,
        sourceStyles.y,
        sourceStyles.zindex,
        sourceStyles.rotate,
